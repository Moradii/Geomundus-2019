---
title: Retrieve and process satellite images time series towards data analysis in R
author: Montesino-SanMartin, M., Perez-Goya, U., Moradi, M., Ugarte, M.D., Militino, A.F.
date: "UPNA"
output:
  html_document:
    theme: cosmo
...
`MODISTools` or `satellite`). So, why developing a new package?
  ...
  resolution of remotely sensed information (e.g., data fusion techniques
  ...
  and offer a *consistent experience* to the user.
  ...
  to develop a *comprehensive* package that facilitates the use of 
...
satellite program and purpose:
  ...
  2. The *suffix* reflects the functionality:
  ...
  - **Process**: cleaning data
    ...
    * `Composition`: generate regular series and improve image quality (e.g.,
    @mosaic1998).
...
in this list. Right now, the package is focused on multispectral images.
...
The package interacts with the **EarthData** and **SciHub** web services. Web
...
following code downloads the shape-file of Spain and its provinces:
...
`RGISTools` and used for visualization. The `tmap` package supports many spatial
data formats, is flexible, and allows interactive and static maps with the same
code.
...
our ROI and time-period. We can extract useful information from the results.
...
x 18 years). The dates confirm that temporal frequency of this product is 16
days. 
...
These 4 images take around 51 MB and 150 MB with the `HDF` and `GeoTiff` files!
To be more efficient, we can specify:
...
We went from 201 MB to 11 MB. Note that the second time, the function runs faster.
...
### Exercise
  ...
  3. Remember to filter the bands that are extracted (`"LST_Day_1km"`) and
...
You will notice the new folder that has been added to the current working
directory. Now, the folders are:

  * `./exercises/ndvi/hdf`: has no image.
  * `./exercises/ndvi/tif`: contains one band.
  * `./exercises/ndvi/castellon`: contains the mosaicked image.

Thanks to cropping, now the size of memory storage consumed by our data-set is
roughly 0.36 MB.
...
It is necessary to re-scale the image to the [-1,1] NDVI range. Information
about the scaling parameter can be found in the 
[product catalogue](https://lpdaac.usgs.gov/products/mod13a2v006/).
...
`RGISTools` has the `genPlotGIS()` function to visualize your data. It is a
wrapper function of `tmap` [@tmap2018], configured to rapidly display the
spatial data usually handled within the package. The basic inputs are:
...
### Exercise
  ...
  2. Find out whether the image must to be re-scaled and, if so, define and 
...
## Concluding remarks
  ...
  * As an introduction, we just downloaded and customized LST and NDVI images
  from MODIS for two locations: Castellon (Spain) and Nordhrein-Westfalen 
  (Germany).
  ...
  * It is crucial to develop a clear hierarchy for the files and folders 
  created along the working steps.
  ...
  * The walk-through and exercises cover the download and customization, and
  does not cover the processing (cloud masking, compositing, and
  filling-smoothing).
